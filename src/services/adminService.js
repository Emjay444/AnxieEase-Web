...
export const adminService = {
  ...
  // Check if a user exists in the admin_profiles table
  ...
  // Add a user to the admin_profiles table
  async addUserToAdminsTable(userId, email, fullName = null) {
    try {
      if (!userId || !email) {
        throw new Error("User ID and email are required");
      }

      // Check if user already exists in admin_profiles table
      ...
      console.log("User successfully added to admin_profiles table:", email);
      return {
        success: true,
        message: "User added to admin_profiles table successfully",
      };
    } catch (error) {
      console.error("Error in addUserToAdminsTable:", error);
      return { success: false, error: error.message };
    }
  },

  // Remove a user from the admin_profiles table
  async removeUserFromAdminsTable(userId) {
    try {
      if (!userId) {
      ...
      if (error) {
        console.error("Error removing user from admins table:", error);
        throw error;
      }

      console.log(
        "User successfully removed from admin_profiles table:",
        userId
      );
      return {
        success: true,
        message: "User removed from admin_profiles table successfully",
      };
    } catch (error) {
      console.error("Error in removeUserFromAdminsTable:", error);
      return { success: false, error: error.message };
    }
  },

  // Promote current user to admin (adds to admins table and updates metadata)
  async promoteToAdmin(email = null) {
    try {
      const {
        data: { user },
        error: userError,
      ...
      if (email && user.email !== email) {
        throw new Error(
          "Email mismatch - please log in with the correct account"
        );
      }

      console.log("Promoting user to admin:", user.email);

      // First, add user to admin_profiles table
      const addToTableResult = await this.addUserToAdminsTable(
        user.id,
        user.email,
        user.user_metadata?.full_name
      );
      if (!addToTableResult.success) {
        throw new Error(addToTableResult.error);
      }

      // Then, update user metadata to include admin role
      const { data, error } = await supabase.auth.updateUser({
        data: {
          role: "admin",
          ...user.user_metadata, // preserve other metadata
      ...
      if (error) {
        // If metadata update fails, try to rollback the admin_profiles table insert
        console.error("Failed to update metadata, attempting rollback");
        await this.removeUserFromAdminsTable(user.id);
        throw error;
      }
  ...
  async setupAdmin(userId, email) {
    try {
      ...
      const addResult = await this.addUserToAdminsTable(userId, email);
