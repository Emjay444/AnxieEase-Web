  ...
  const confirmAppointmentAction = async () => {
    ...
    try {
      ...
      const ok = await appointmentService.updateAppointmentStatus(
        requestId,
        status,
        action === "approve" ? "Approved by psychologist" : "Declined by psychologist"
      );
      if (ok) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {showPatientProfile && selectedPatient ? (
        ...
        <>
          ...
          {/* Schedule Appointment Modal (independent) */}
              ...
              <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-700">
                    Pick a time on the calendar, then click “Confirm” to
                  ...
                  {selectedSlot?.start && (
                    <div className="p-3 rounded-lg bg-emerald-50 text-sm text-emerald-800">
                      ...
                      <div>{new Date(selectedSlot.start).toLocaleString()}</div>
                    </div>
                  )}
                </div>
          ...
          {showConfirmModal && pendingAction && (
            <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-700">
                    Are you sure you want to {pendingAction.action} this appointment request?
                  </p>
                  <div className="p-3 rounded-lg bg-gray-50 text-sm text-gray-700">
                    <div className="font-medium">
                      {pendingAction.action === 'approve' ? 'This will:' : 'This will:'}
                    </div>
